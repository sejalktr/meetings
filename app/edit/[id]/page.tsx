    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="max-w-xl mx-auto mt-8 px-4">
        <form onSubmit={handleUpdate} className="space-y-6">
          {/* TWO PHOTO GALLERY SECTION */}
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
             <div className="grid grid-cols-2 gap-6">
                <div className="flex flex-col items-center">
                    <img src={formData.photo_1 || 'https://via.placeholder.com/150'} className="w-full aspect-square rounded-[2rem] object-cover border-4 border-white shadow-md mb-4" />
                    <InputField label="Photo 1 URL" name="photo_1" defaultValue={formData.photo_1} />
                </div>
                <div className="flex flex-col items-center">
                    <img src={formData.photo_2 || 'https://via.placeholder.com/150'} className="w-full aspect-square rounded-[2rem] object-cover border-4 border-white shadow-md mb-4" />
                    <InputField label="Photo 2 URL" name="photo_2" defaultValue={formData.photo_2} />
                </div>
             </div>
          </div>
          ...
          <button type="submit" disabled={saving} className="w-full py-5 bg-indigo-600 text-white rounded-[2rem] font-bold shadow-xl uppercase italic">
  ...
  return (
    <div className="space-y-1 w-full">
      ...
      <div className="relative">
        ...
        <input name={name} type={type} defaultValue={defaultValue} className={`w-full ${icon ? 'pl-11' : 'pl-4'} pr-4 py-4 bg-slate-50 border-none ring-1 ring-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 text-slate-700 font-medium`} />
      </div>
    </div>
  );
}
...
export default function EditPageWrapper({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Suspense fallback={<div className="p-20 text-center">Loading Router...</div>}>
