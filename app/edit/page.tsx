...
// --- YOUR ORIGINAL EDITFORM COMPONENT (Logic Fixed) ---
function EditForm({ token }: { token: string }) {
  ...
  if (!formData) return (
    <div className="text-center mt-20">
      <h2 className="text-xl font-bold text-red-500 uppercase italic">Invalid or Expired Edit Link</h2>
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="bg-white border-b sticky top-0 z-10 p-4">
        <div className="max-w-xl mx-auto flex items-center gap-4">
          ...
          <h1 className="text-xl font-black text-slate-800 uppercase italic">Edit Profile</h1>
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto mt-8 px-4">
        <form onSubmit={handleUpdate} className="space-y-6">
          ...
          <button 
            type="submit" 
            disabled={saving}
            className="w-full py-5 bg-indigo-600 text-white rounded-[2rem] font-bold shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 uppercase italic"
          >
...
// YOUR ORIGINAL HELPER COMPONENT
...
// --- THE FIX: WRAPPER NOW PASSES THE URL ID TO YOUR FORM ---
export default function EditPageWrapper({ params }: { params: Promise<{ id: string }> }) {
  // We "use" the promise to get the ID from the folder name [id]
  const resolvedParams = use(params);
  
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen font-bold uppercase italic text-slate-400">Loading Router...</div>}>
